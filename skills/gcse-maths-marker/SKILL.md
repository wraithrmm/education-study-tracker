...
A complete run needs three things. If any is missing, ask for it before marking:
...
- **`For-Student_<paper>-<series>.md`** — the copy handed to the student. It must **NOT contain the correct answers** — the point is for the student to re-work the questions themselves. Include: a summary block (mark, %, grade, tier), a compact per-question marks grid, a "where you lost marks — what to review" table giving the **topic and a method hint only, never the answer**, and a prioritised list of revision topics.  See `examples/for-student-example.md`.
...
The revision-gaps section is the point of the exercise — cluster losses into topics (e.g. "straight-line graphs", "solving equations", "probability") so the student can be given focused practice. Also note whether blank back-of-paper questions look like a **timing/pace** problem rather than a knowledge gap.
